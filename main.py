...
for run in range(N_RUNS):
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        train_gt, test_gt = sample_gt(gt, train_size=SAMPLE_PERCENTAGE, mode=SAMPLING_MODE)
    print(
    ...
    if MODEL == "SVM_grid":
        ...
        with torch.no_grad():
            ...
            summary(model.to(hyperparams["device"]), input.size())
        ...
        probabilities= test(model, img, hyperparams)  # (H, W, classes)
        num_cls = int(probabilities.shape[2])

        prediction1 = np.argmax(probabilities, axis=-1)  # (H, W)
        prediction2 = np.argmax(probabilities, axis=-1)

        # plt_test = tsne(probabilities, num_cls)
        # viz.matplot(plt_test)
