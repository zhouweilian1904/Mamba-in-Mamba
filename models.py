...
from my_models.vit_3d_1_92_8835 import ViT3d_1
from my_models.vit_3d_2 import ViT3d_2 as ViT3d_2
from my_models.vit_3d_RViT import RVIT as RVIT_3d
from my_models.TGRS_3_demo_Salinas_v1 import RVIT_Salinas as RVIT_Salinas_v1
from my_models.TGRS_3_demo_Salinas_v2 import RVIT_Salinas as RVIT_Salinas_v2
from my_models.SSFTTnet import SSFTTnet
from my_models.TGRS_2 import multiTrans
from my_models.yang import Yangnew
from my_models.OneDRNN import OneDRNN
from my_models.FourDRNN import FourDRNN
from my_models.EightDRNN import EightDRNN
from my_models.EightDLSTM import EightDLSTM
from my_models.general_vit import General_ViT
from my_models.general_vit_2 import General_ViT_2
from my_models.Speformer import Speformer
from my_models.Spectral_attention_ICASSP import SpectralAttention
from my_models.ICPR2022 import ICPR2022
from my_models.SSViT import SSViT
from my_models.SS3d_ICIP import ICIP2022
...
from my_models.RCNN import RCNN
from my_models.conv2d import TESTEtAl
from my_models.conv3d import C3DEtAl
from my_models.Involution3 import I2DEtAl, I3DEtAl
from vit_pytorch.vipp import ViP
from vit_pytorch.dwt import DWT, ConvPermute
from my_models.mamba_1 import Mamba_1 as mamba1
from my_models.MiM_v3 import MiM_v3
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        center_pixel = True
        model = LocalViT(dim=128, image_size=15, patch_size=3, depth=6, heads=16, mlp_dim=2048, dropout=0.1,
                         emb_dropout=0.1, num_classes=n_classes, channels=n_bands, )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = T2TViT(dim=512, image_size=15, depth=4, heads=8, mlp_dim=512, t2t_layers=((7, 2), (3, 2), (3, 2)),
                       dropout=0.1, emb_dropout=0.1, num_classes=n_classes, channels=n_bands, )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = RvT(dim=128, image_size=15, patch_size=3, depth=6, heads=16, mlp_dim=2048, dropout=0.1, emb_dropout=0.1,
                    num_classes=n_classes, channels=n_bands, )
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = PiT(dim=256, image_size=15, patch_size=3, depth=(3, 3, 3, 3), heads=16, mlp_dim=2048, dropout=0.1,
                    emb_dropout=0.1, num_classes=n_classes, channels=n_bands, )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = DeepViT(dim=1024, image_size=15, patch_size=3, depth=6, heads=16, mlp_dim=2048, dropout=0.1,
                        emb_dropout=0.1, num_classes=n_classes, channels=n_bands, )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = LeViT(dim=(256, 384, 512), image_size=224, stages=3, depth=4, heads=(4, 6, 8), mlp_mult=2, dropout=0.1,
                      num_classes=n_classes, channels=n_bands, )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = TNT(image_size=15, patch_dim=512, pixel_dim=24, patch_size=3, pixel_size=3, depth=6, heads=16,
                    attn_dropout=0.1, ff_dropout=0.1, num_classes=n_classes, channels=n_bands, )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        model = VisionPermutator(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims,
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = ViP(pretrained=False, in_chans=n_bands, inplanes=64, num_classes=n_classes, num_chs=(96, 192, 384, 768),
                    patch_sizes=[1, 1, 1, 1], num_heads=[3, 6, 12, 24],
        ...
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [256, 256, 512, 512]
        model = DWT(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3,
        ...
        lr = kwargs.setdefault('lr', 0.001)
        model = ViT3d_1(n_bands, n_classes, patch_size)
        model = model.to(device)
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = ViT3d_2(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = RVIT_3d(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = RVIT_Salinas_v1(n_bands, n_classes, patch_size, batch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
            lambda rec, data: F.l1_loss(
                rec, data.squeeze(), reduction='sum'
            ),
        )
        # criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
        #              lambda rec, data: F.l1_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'RVIT-Salinas-2':
        kwargs['supervision'] = 'full'
        patch_size = kwargs.setdefault('patch_size', 9)
        center_pixel = True
        epoch = kwargs.setdefault('epoch', 200)
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = RVIT_Salinas_v2(n_bands, n_classes, patch_size, batch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        # criterion = (
        #     nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = General_ViT(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = General_ViT_2(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = Speformer(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        kwargs.setdefault('batch_size', 100)
        # lr = kwargs.setdefault('lr', 1.0)
        model = SpectralAttention(n_bands, n_classes)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adam(model.parameters())
        criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']), lambda rec, data: F.mse_loss(rec, data.squeeze()))
    elif name == 'One-Direction-RNN':
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = True
        kwargs.setdefault('epoch', 200)
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = OneDRNN(n_bands, n_classes, patch_size=patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters())
        # criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']),
                     lambda rec, data: F.mse_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'Four-Direction-RNN':
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = True
        kwargs.setdefault('epoch', 200)
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = FourDRNN(n_bands, n_classes, patch_size=patch_size)
        model = model.to(device)
        optimizer = optim.Adam(model.parameters())
        # criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
                     lambda rec, data: F.mse_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'Eight-Direction-Trans':
        kwargs['supervision'] = 'full'
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = True
        kwargs.setdefault('epoch', 200)
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = EightDRNN(n_bands, n_classes, patch_size=patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters())
        # criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
                     lambda rec, data: F.mse_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'Eight-Direction-LSTM':
        kwargs['supervision'] = 'full'
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = True
        kwargs.setdefault('epoch', 200)
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = EightDLSTM(n_bands, n_classes, patch_size=patch_size)
        model = model.to(device)
        optimizer = optim.Adam(model.parameters())
        # criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
        ...
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = ICPR2022(n_bands, n_classes)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters())
        # criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        criterion = (
        ...
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = ICIP2022(n_bands, n_classes)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters())
        # criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        criterion = (
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = mamba1(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        batch_size = kwargs.setdefault('batch_size', 64)
        lr = kwargs.setdefault('lr', 0.001)
        model = mamba2(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        patch_size = kwargs.setdefault('patch_size', 9)
        center_pixel = True
        lr = kwargs.setdefault('lr', 0.0005)
        model = MiM_v1(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        center_pixel = True
        lr = kwargs.setdefault('lr', 0.0005)
        model = MiM_v2(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        center_pixel = True
        lr = kwargs.setdefault('lr', 0.0005)
        model = MiM_v3(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
...
class LiuEtAl(nn.Module):
    ...
    def forward(self, x):
        x = x.squeeze(1)
        # print('x',x.shape)
        x = x.view(-1, self.input_channels, self.patch_size, self.patch_size)
...
def train(
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network", colour='red'):
        ...
        for batch_idx, (data, target) in tqdm(
                enumerate(data_loader), total=len(data_loader), colour='blue'
        ):
...
def test(net, img, hyperparams):
    ...
    """
    net.eval()
    # print('img size', img.shape) #（H,W,C)
    ...
    # regs = np.zeros(img.shape[:2] + (img.shape[2],))
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            ...
            if patch_size == 1 or center_pixel:
                ...
                # reg = np.transpose(reg.numpy(), (0, 2, 3, 1))
            ...
            # for (x, y, w, h), reg in zip(indices, reg):
            #     if center_pixel:
            #         regs[x + w // 2, y + h // 2] += reg
            #     else:
            #         regs[x: x + w, y: y + h] += reg

            # print('probs size', probs.shape)
            # print('regs size', regs.shape)
        # file_path = 'Datasets/regression data.mat'
        # # Create a dictionary to store your data
        # # The keys in this dictionary will be the variable names in MATLAB
        # mat_dict = {'regs': regs}
...
def val(net, data_loader, device="cpu", supervision="full"):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        with torch.no_grad():
            ...
            if supervision == 'full':
                outputs = net(data)
                output = outputs[0] if len(outputs) == 2 else outputs
            elif supervision == 'semi':
                output, rec = net(data)
