...
class ViT3d_2(nn.Module):
    ...
    def __init__(self, n_bands, n_classes, patch_size, subimage_patch_size=subimage_patch_size,
        ...
        self.partial_rnn = nn.ModuleList(
            [VisionEncoderMambaBlock(dim=self.patch_dim, dt_rank=self.patch_dim, dim_inner=self.patch_dim, d_state=self.patch_dim)
             for _ in range(self.num_spatial)])
        ...
        self.vim2 = VisionEncoderMambaBlock(dim=dim, dt_rank=dim, dim_inner=dim, d_state=dim)
    ...
    def one_direction_operation(self, x):
        ...
        x = self.dropout(x)
        x = self.vim(x)  # (b, num_h*num_w*num_f, dim)
        x = self.layernorm(x)
