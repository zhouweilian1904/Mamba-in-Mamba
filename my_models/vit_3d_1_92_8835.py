...
def _generate_G_from_H(S, W_node_edge):
    """
...
class Attention(nn.Module):
    def __init__(self, dim, num_f, heads, dim_head, dropout, attn_type):
        super().__init__()
    ...
    def spe_dis_mat_3d(x):
        ...
        spe_pairwise_weights = torch.where(spe_pairwise_weights < torch.mean(spe_pairwise_weights),torch.zeros_like(spe_pairwise_weights), spe_pairwise_weights)
    ...
    def SMSA3d(self, x):
        ...
        spe_dis = repeat(spe_dis, 'b n1 n2 -> b h n1 n2', h=h, b=b)
    ...
    def SMSA2d(self, x):
        ...
        spe_mask = repeat(spe_mask, 'b n1 n2 -> b h n1 n2', h=h, b=b)
        spe_mask = spe_mask.float()
        spa_mask = self.spa_dis_mat_2d(x)
        spa_mask = repeat(spa_mask, 'b n1 n2 -> b h n1 n2', h=h, b=b)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, num_f, dropout=0.2, attn_type='SMSA3d'):
        super().__init__()
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, LocalRNNLayer(input_dim=dim, output_dim=dim, rnn_type='LSTM', ksize=1, stride=1,
                                           dropout=dropout)),
...
class Transformer2(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, num_f, dropout=0.1, attn_type='Original'):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, nn.LSTM(input_size=dim, hidden_size=dim, num_layers=depth, batch_first=True, bias=True,
                                     bidirectional=False)),
                PreNorm(dim, Attention(dim, num_f=num_f, heads=heads, dim_head=dim_head, dropout=dropout,
                                       attn_type=attn_type)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout=dropout)),
...
class ViT3d_1(nn.Module):
    ...
    def __init__(self, n_bands, n_classes, patch_size, subimage_patch_size=subimage_patch_size,
        ...
        # self.partial_rnn = nn.LSTM(input_size=self.patch_dim, hidden_size=self.patch_dim, num_layers=1, batch_first=True, bias=True, bidirectional=False)
        ...
        self.conv = nn.Conv2d(n_bands, n_bands, 2, 1, padding=(1, 1), bias=True, groups=self.num_f)
        self.patch_dim_to_dim = nn.Linear(self.patch_dim, dim, bias=True)
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, self.num_f, dropout,
                                       attn_type='SMSA3d')
        self.transformer2 = Transformer2(dim, depth, heads, dim_head, mlp_dim, self.num_f, dropout,
                                         attn_type='Original')
        ...
        self.relu = nn.ReLU6()
        self.a_weights = nn.Parameter(torch.randn(8))
        self.b_weights = nn.Parameter(torch.randn(8))
    ...
    def forward(self, x):
        ...
        out_4, reg_4 = self.one_direction_operation(x_dir_4)
        ...
        a_weights = F.softmax(self.a_weights, dim=0)
        b_weights = F.softmax(self.b_weights, dim=0)
        return (self.mlp_head(a_weights[0] * out_1 + a_weights[1] * out_2 + a_weights[2] * out_3 + a_weights[3] * out_4
                              + a_weights[4] * out_5 + a_weights[5] * out_6 + a_weights[6] * out_7 + a_weights[
                                  7] * out_8),
                self.reg(b_weights[0] * reg_1 + b_weights[1] * reg_2 + b_weights[2] * reg_3 + b_weights[3] * reg_4
                         + b_weights[4] * reg_5 + b_weights[5] * reg_6 + b_weights[6] * reg_7 + b_weights[7] * reg_8))
