...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        B, L, C = x.shape
        x = x.permute(0, 2, 1)  # Change to [B, C, L] for Conv1d

        max_features, _ = torch.max(x, 2, keepdim=True)
        avg_features = torch.mean(x, 2, keepdim=True)
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x_):
        ...
        # y_cat = self.pro_to_cat(y_cat)
        # y_mask = F.normalize(self.gaussian_mask_index(y_cat, 'center') * self.gaussian_mask_vector(y_cat, 'center'), p=2)
        # y = y_cat * y_mask.unsqueeze(-1)
...
class T_Mamaba(nn.Module):
    def __init__(self, channels, image_size, patch_size=1, dim=128, depth=1, emb_dropout=0.
        ...
        self.mamba_config = MambaConfig(d_model=dim, n_layers=depth, seq_length=self.num_patches,
                                        dropout=emb_dropout, d_state=16, bias=True, num_tokens=num_tokens)
    ...
    def forward(self, img):
        ...
        x = self.mamba(x)  # (100, num_tokens, 64)
        # print('x', x.shape)
...
class MiM_main(nn.Module):
    ...
    def forward(self, x):
        ...
        # O_2 = torch.mean(tm1_2 + tm2_2 + tm3_2 + tm4_2, dim=1)

        # tm1_3, tm2_3, tm3_3, tm4_3 = self.mim_3(tm1_2, tm2_2, tm3_2, tm4_2)

        # O_3 = torch.mean(tm1_3 + tm2_3 + tm3_3 + tm4_3, dim=1)

        # tm1_4, tm2_4, tm3_4, tm4_4 = self.mim_4(tm1_3, tm2_3, tm3_3, tm4_3)

        # O_4 = torch.mean(tm1_4 + tm2_4 + tm3_4 + tm4_4, dim=1)
